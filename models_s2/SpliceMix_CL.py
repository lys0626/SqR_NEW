...
#动态ASL loss
# class Loss_fn(nn.Module):  # 不再继承 loss_fns.BCELoss
#     def __init__(self):
#         super(Loss_fn, self).__init__()
        
#         # 1. 主分类损失：启用 D-ASL 动态把关
#         # (你可以根据需要调整 gamma_neg, max_clip 的值，当前使用你库中的默认参数)
#         self.main_loss = DynamicAsymmetricLoss(gamma_neg=4, gamma_pos=1, base_clip=0.05, max_clip=0.2)
        
#         # 2. 一致性学习损失：保留标准的 SoftMarginLoss (等价于 BCE) 用于局部特征对齐
#         self.cl_loss = nn.MultiLabelSoftMarginLoss()

#     def forward(self, inputs, targets_ignored, progress_ratio=0.0):
#         # 接收外部 engine.py 传进来的 progress_ratio
#         if isinstance(inputs, tuple) and len(inputs) == 4:
#             preds_all, preds_m, preds_m_r, targets_all = inputs
            
#             # 1. 主分支：使用 D-ASL，并施加时间沙漏 progress_ratio
#             loss_bce = self.main_loss(preds_all, targets_all, progress_ratio=progress_ratio)
            
#             # 2. CL 分支：对齐深层特征的网格碎块，使用标准 BCE 即可
#             loss_cl = self.cl_loss(preds_m, preds_m_r.sigmoid().detach())
            
#             return loss_bce + loss_cl
            
#         elif isinstance(inputs, tuple) and len(inputs) == 2:
#             preds_all, targets_all = inputs
#             return self.main_loss(preds_all, targets_all, progress_ratio=progress_ratio)
            
#         # 测试阶段或完全未触发 mix 时
#         return self.main_loss(inputs, targets_ignored, progress_ratio=progress_ratio)
