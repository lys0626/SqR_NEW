...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
                 train_backbone: bool,#是否训练backbone网络的参数
                 return_interm_layers: bool,#是否返回中间层的特征，True返回多层特征，False只返回最后一层特征
                 dilation: bool,#是否在最后一层使用空洞卷积
                 pretrained: bool = True):
        if name in ['resnet18', 'resnet50', 'resnet34', 'resnet101']:
            #根据字符串名称name动态获取torchvision中的模型类
            #如果name是'resnet50'，则相当于执行torchvision.models.resnet50()
            backbone = getattr(torchvision.models, name)(
                replace_stride_with_dilation=[False, False, dilation],#是否用空洞卷积代替步长下采样
                pretrained=True,
                norm_layer=FrozenBatchNorm2d)#冻结批归一化层的参数,防止模型训练崩溃，因为在Transformer任务中,批量大小通常较小，导致BatchNorm层的统计数据不稳定
            #设定要提取哪些层的特征
            if return_interm_layers:
                return_layers = {"layer1": "0", "layer2": "1", "layer3": "2", "layer4": "3"}
            else:
            #如果只需要最后一层特征（通常用于 Query2Label），只提取 layer4
                return_layers = {'layer4': "0"}
...
class Joiner(nn.Sequential):
    def __init__(self, backbone, position_embedding, args=None):
        #调用父类的初始化方法，把backbone和position_embedding作为子模块添加进来，backbone注册为self[0]，position_embedding注册为self[1]
        super().__init__(backbone, position_embedding)
        # self.args = args
        # if args is not None and 'interpotaion' in vars(args) and args.interpotaion:
        #     self.interpotaion = True    #开启插值模式,用于处理位置编码尺寸不匹配的情况
        # else:
        #     self.interpotaion = False   #默认关闭


    def forward(self, input: Tensor):
        #获取骨干网络输出的字典
        xs = self[0](input)
        out: List[Tensor] = []# : 类型注解的分隔符，用来指定变量的类型
        pos = []
        #如果backbone输出的是一个字典(例如ResNet)，在Query2label情况下，虽然是字典，但是只有一个键值对，value为最后一层的特征
        if isinstance(xs, dict):
            for name, x in xs.items():
                out.append(x)
                # position encoding
                pos.append(self[1](x).to(x.dtype))
        #如果backbone输出的是张量(例如Swin Transformer)，则直接处理该输出
        else:
...
def build_backbone(args):
    ...
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        ...
        if args.pretrained:
            ...
            '''
                _tmp_st_output 是一个包含两部分信息的对象：
                missing_keys (缺失的键)：模型里有，但在权重文件里没找到的层。
                unexpected_keys (多余的键)：权重文件里有，但模型里不需要的层。
            '''
            print(str(_tmp_st_output))
        #修改前向传播函数，使其只输出特征提取部分的结果，也就是获取卷积层最后面的输出特征图
        backbone.forward = backbone.forward_features
        #由swin-transformer决定，每经过一个阶段，特征图的长宽减半，但是通道数翻倍
